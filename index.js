...
//message insert
app.post("/api/messagei", (req, res) => {
    const sid = req.body.sid;
    const name = req.body.name;
    const message = req.body.message;
   console.log(message)

    const sqlQuery = "INSERT INTO messages (sid, name, message) VALUES (?, ?, ?)";
    db.query(sqlQuery, [sid, name, message], (err, result) => {
        if (err) {
            res.status(500).json({ error: "An error occurred while processing your request" });
            console.log(err);
        } else {
...
//update status
app.post("/api/updatestatuss", (req, res) => {
    
    const idmessages = req.body.idmessages;

    const sqlQuery = "UPDATE messages SET status = 'done' WHERE idmessages = ?";
    db.query(sqlQuery, [idmessages], (err, result) => {
        if (err) {
            res.status(500).json({ error: "An error occurred while processing your request" });
            console.log(err);
        } else {
...
app.post("/api/messages", (req, res) => {
    ...
    const sqlQuery = "select * from messages where  status=?";
    db.query(sqlQuery, [name], (err, result) => {
        res.send(result);
...
app.post('/api/ticketupdate/:id', (req, res) => {
  ...
  // Define a route to serve the SVG file
  app.get('/upload/images.jpeg', (req, res) => {
    // Assuming 'images.jpeg' is located in the 'upload' directory
    res.sendFile(path.join(__dirname, 'upload', 'images.jpeg'));
});
